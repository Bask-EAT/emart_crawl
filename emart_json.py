...
import urllib.parse  # URL 파싱을 위해 urllib.parse 모듈 추가
...
def scrape_emart_category_page(html_content, category_name):
    ...
    if product_list_ul:
        ...
        print(
            f"경고: '{category_name}' 카테고리에서 '#ty_thmb_view > ul' 태그를 찾을 수 없습니다. 웹사이트 구조가 변경되었을 수 있습니다."
        )
    ...
    if len(product_items) == 0:
        print(
            f"경고: '{category_name}' 카테고리에서 'li' 태그를 가진 상품 항목을 찾을 수 없습니다. 웹사이트 구조가 변경되었거나 콘텐츠가 동적으로 로드될 수 있습니다."
        )
    # --- 디버깅을 위한 추가 코드 끝 ---
    ...
    for category_name, disp_ctg_id in categories_to_scrape.items():
        ...
        try:
            for page_num in range(start_page, end_page + 1):
                ...
                time.sleep(2)
            # 전체 정보 저장 (result_json 폴더, 넘버링 없이)
            output_file = f"result_json/{category_name}_emart_products.json"
            with open(output_file, "w", encoding="utf-8") as f:
                json.dump(all_scraped_products_for_category, f, ensure_ascii=False, indent=4)
            print(f"전체 정보 저장: {output_file}")
